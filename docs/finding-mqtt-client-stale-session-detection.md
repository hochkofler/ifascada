...
**Status:** Partially fixed — `central-server`'s consumer (`crates/central-server/src/mqtt_consumer.rs`) now self-heals via an application-level staleness watchdog (see "Fix implemented — 2026-08-25", suggested fix #2); the `edge-agent` side (`crates/edge-agent/src/mqtt_bridge.rs`) is **still open**, deliberately out of scope for the web-ui-v2 plan that produced the central-side fix. Suggested fixes #1, #3 and #4 remain unimplemented on both sides.
...
## Fix implemented — 2026-08-25: central-server staleness watchdog (suggested fix #2)

**Scope:** `crates/central-server/src/mqtt_consumer.rs` only. `crates/edge-agent/src/mqtt_bridge.rs` is a separate binary/deployment target (runs on edge devices in the field) and was deliberately left untouched — the edge side of this finding stays open.

**What was built.** A `BrokerActivityWatch` value type that owns one question — "how long since we last heard *from the broker*" — plus the wiring that lets `run_mqtt_consumer`'s poll loop ask it:

- **Bound:** silence longer than `keep_alive * 1.5` (with the consumer's `keep_alive` of 20s, that is **30s**) means the session is presumed dead. This is the bound named in suggested fix #2 above.
- **What counts as activity:** *inbound packets only* (`Event::Incoming(_)` — `ConnAck`, `SubAck`, `PubAck`, `Publish`, and critically `PingResp`). `Event::Outgoing(_)` deliberately does not count: outgoing events are produced by our own writes, and writing into a half-open socket's send buffer keeps succeeding, which is precisely why the pre-fix loop could look busy while nothing was arriving.
- **How the check gets a chance to run:** `eventloop.poll()` is now wrapped in `tokio::time::timeout(...)`. Without that, an idle poll on a dead socket parks indefinitely and no amount of elapsed-time logic ever executes. The timeout is the *remaining* time to the staleness deadline, never a fixed tick, so it can only expire at a moment when the connection is going to be torn down anyway — cancelling `poll()` mid-flight therefore can never leave a still-in-use connection half-written. A 1s floor keeps the loop from spinning.
- **What happens on detection:** a `warn!` naming the observed silence, then `break` out of the inner loop — reusing the existing reconnect-with-backoff path verbatim. The change is purely **additive**: the pre-existing `Err(e)` arm (real `poll()` errors) is untouched and still reconnects exactly as before. Activity is stamped *after* the ingest handler returns, so a slow database cannot be misread as a dead broker.

**Tests.** 10 pure unit tests over the decision logic (`cargo test -p central-server --lib mqtt_consumer`), covering the boundary explicitly: at exactly `keep_alive * 1.5` the broker still gets the benefit of the doubt, strictly beyond it does not; activity resets the window; the window scales with `keep_alive`; a backwards-moving clock never forces a reconnect; and the first check interval is longer than `rumqttc`'s own 5s connect timeout, so the watchdog can never cancel an in-flight CONNECT. Full crate suite after the change: **35 passed, 0 failed, 1 ignored** (the ignored one is the live repro below); `cargo check --package central-server` clean, no warnings.

**Live repro — this finding's suggested fix #5, now automated for the central client.** Committed as `mqtt_consumer.rs`'s `#[ignore]`d `live_repro::watchdog_detects_frozen_broker_within_1_5x_keep_alive` (the module's doc comment carries the exact `docker run` invocation). Method: point a real `rumqttc` client at a **throwaway** `eclipse-mosquitto:2` on port 51884 (never the shared dev stack's `ifascada-mosquitto` — the test SIGSTOPs whatever container it targets), let the session fully establish, then `docker pause` the broker. A paused container is frozen by the cgroup freezer while its kernel networking stays up, so the client receives **no FIN and no RST** and no `PINGRESP` — the same shape as this finding's original `iptables -j DROP` sketch, without needing a firewall rule.

Measured on `rumqttc` 0.24, both runs against the same frozen broker (last inbound packet was a `PingResp` at t+20.1s, immediately after the freeze):

| Loop under test | Detection | Notes |
|---|---|---|
| Plain `eventloop.poll()` (pre-fix shape) | `poll()` returned `Err` at **t+60.2s** — **40s** after the last inbound packet | `Mqtt state: Last pingreq isn't acked`, i.e. `rumqttc`'s internal `AwaitPingResp` guard firing on the *second* keep alive tick (2 × 20s) |
| `BrokerActivityWatch` + timeout-wrapped `poll()` (the fix) | watchdog fired at **t+50.1s** — **30.0s** after the last inbound packet, exactly `keep_alive * 1.5` | logged `mqtt broker silent for 30s while polling`, then reconnected via the existing backoff path |

**Honest limits of that evidence.** This repro reproduces a *silent* frozen session and shows the fix bounding detection at 30s instead of 40s — but it did **not** reproduce production's headline symptom of detection *never* happening (the 73-minute gap on 2026-08-18): here `rumqttc` 0.24's own guard did eventually fire, just later. So the exact reason the production client stayed silent for 73 minutes remains unexplained by this test — candidate explanations still open are a half-open socket state in which even the `AwaitPingResp` guard never runs (the keep alive branch losing `rumqttc`'s internal `select!` indefinitely), a different `rumqttc` version in the deployed binary, or the edge's own distinct code path. What this fix does guarantee is that central no longer *depends* on a third-party state machine's internal guard to notice: the bound is now owned, explicit, tested, and tighter. The production incident's edge side (`.154`, `mqtt_bridge.rs`) has no equivalent guard yet.

**Not addressed (still open):** suggested fix #1 (OS-level TCP keepalive on the socket), #3 (only log `mqtt publish ok` after a real `PUBACK`), #4 (whether central's `clean_session=false` is buying anything), and all of the above on the `edge-agent` side. Also unaddressed and separate: a session where MQTT keep alive is healthy but the *subscription* is gone — the broker answers every `PINGREQ` yet delivers no publishes — still looks alive to this watchdog, because a `PINGRESP` is inbound activity.
